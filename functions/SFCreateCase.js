...
exports.handler = async function (context, event, callback) {
  ...
  let create_url = "https://" + process.env.SFDC_INSTANCE_URL + "/services/data/v46.0/sobjects/Case/";
  ...
  try {
    if (event.bearer_token) {
      console.log(`CREATE CASE ALREADY HAVE A BEARER TOKEN`);
  ...
  try {
    // console.log(`Querying SFDC with query: ${create_url} and data: ${JSON.stringify(requestPayload,null,4)}`);
    const response = await axios.post(create_url, requestPayload);
